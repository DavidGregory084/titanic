...
object Prune {
  ...
  /**
    ...
    */
  def build: Input => AttrF[Counts, Input] = {
  ...
  /**
    ...
    */
  def minCost: AttrF[CostInfo, Double] => Double = {
  ...
  /**
    ...
    * 
    */
  val prunedSubtrees: Fix[AttrFCostInfo] => ListF[Fix[AttrFCostInfo], Fix[AttrFCostInfo]] = {
    case AttrF(_, Leaf(_)) => NilF()
    case tree =>
      val subtree = Schemes.cata(tree)(prune(Schemes.cata(tree)(minCost)))
  ...
  def bestSubtree(tree: Fix[AttrFCostInfo], validationData: List[TestDataRow],
    validationLabels: Map[Int, Label]):
      ListF[Fix[AttrFCostInfo], (Double, Tree)] => (Double, Tree) = {
  case NilF() =>
      val untagged = Schemes.cata(tree)(AttrF.untag)
  ...
  case ConsF(subtree, (currentMin, current)) =>
      ...
      val untagged = Schemes.cata(t)(AttrF.untag)
    val risk = assess(untagged)(validationData, validationLabels).risk
